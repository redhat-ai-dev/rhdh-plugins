...
import { tsControllerCursorPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [tsControllerCursorPlugin],
    });

    await request(server).get('/api/ts-controller-cursor/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/ts-controller-cursor/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/ts-controller-cursor/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/ts-controller-cursor/todos/${createdTodoItem.id}`)
