...
import { informerTsViaGeminiPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [informerTsViaGeminiPlugin],
    });

    await request(server).get('/api/informer-ts-via-gemini/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/informer-ts-via-gemini/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/informer-ts-via-gemini/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/informer-ts-via-gemini/todos/${createdTodoItem.id}`)
